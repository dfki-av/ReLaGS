...
// custom function to perform atomic max operation on float values
...
renderCUDA(
	...
	float* __restrict__ max_contribution)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicMaxFloatContrib(&max_contribution[collected_id[j]], w);
