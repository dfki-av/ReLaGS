...
//   auto uint_opts = means3D.options().dtype(torch::kUInt32);
  ...
  torch::Tensor out_hit_ids = torch::full({1, H, W}, -1, int_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_hit_ids.contiguous().data<int>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_others, radii, out_hit_ids, geomBuffer, binningBuffer, imgBuffer);
}
