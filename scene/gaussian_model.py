...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, embedding_dim : int = 20):
        ...
        self._max_contribution = torch.empty(0)
    ...
    def setMaxContribution(self, max_contribution):
        max_contribution = max_contribution.detach()
        self._max_contribution = torch.max(self._max_contribution, max_contribution)
    ...
    def construct_list_of_attributes(self, save_semantics=False):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
    ...
    def save_ply(self, path, save_semantics=False):
        ...
        if save_semantics:
            ...
            sem = np.zeros((xyz.shape[0], self.embedding_dim), dtype=np.float32)
        opacities = self._opacity.detach().cpu().numpy()
        scale = self._scaling.detach().cpu().numpy()
        rotation = self._rotation.detach().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes(save_semantics)]
    ...
    def load_ply(self, path):
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self._max_contribution = torch.zeros((self.get_xyz.shape[0]), device="cuda")
    ...
    def filter_with_impactful_mask(self, mask):
        valid_points_mask = ~mask
    ...
    def filter_gaussians(self, mask):
        new_model = GaussianModel(self.max_sh_degree)

        new_model._xyz = nn.Parameter(self._xyz[mask].clone().detach())
        new_model._features_dc = nn.Parameter(self._features_dc[mask].clone().detach())
        new_model._features_rest = nn.Parameter(self._features_rest[mask].clone().detach())
        new_model._scaling = nn.Parameter(self._scaling[mask].clone().detach())
        new_model._rotation = nn.Parameter(self._rotation[mask].clone().detach())
        new_model._opacity = nn.Parameter(self._opacity[mask].clone().detach())
        new_model._semantics = nn.Parameter(self._semantics[mask].clone().detach())
