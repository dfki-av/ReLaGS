...
def num_to_natural_torch(group_ids, void_number=-1):
    ...
    if (void_number == -1):
        # [-1,-1,0,3,4,0,6] -> [-1,-1,0,1,2,0,3]
        if torch.all(group_ids == -1):
            return group_ids
        array = group_ids.clone()

        unique_values = torch.unique(array[array != -1])
        mapping = torch.full((torch.max(unique_values).item() + 2,), -1, dtype=torch.int32, device=group_ids.device)
        # map ith(start from 0) group_id to i
        mapping[unique_values + 1] = torch.arange(len(unique_values), dtype=torch.int32, device=group_ids.device)
        array = mapping[array + 1]
        ...
        mapping = torch.full((torch.max(unique_values).item() + 2,), 0, dtype=torch.int32)
        mapping[unique_values] = torch.arange(len(unique_values), dtype=torch.int32) + 1
        array = mapping[array]
    else:
        raise Exception("void_number must be -1 or 0")
...
def torch_calcu_all_similar_optimized(labels1, labels2, thres_trunc=0):
    '''
    labels1: (s1, lr-1)
    labels2: (s2, lr-1)
    '''
    # 直接通过矩阵运算计算欧氏距离平方，避免生成大索引矩阵
    dis_sq = torch.cdist(labels1, labels2, p=2).pow(2)  # (s1, s2)
    
    similar = (2 - dis_sq) / 2.0  # 等价于原similar_func

    if thres_trunc > 0:
        # similar.clamp_min_(thres_trunc)
        # similar = torch.where(similar < thres_trunc, torch.zeros_like(similar), similar)
        similar = torch.where(similar < thres_trunc, torch.tensor(0, device=similar.device), similar)


    return similar

@torch.inference_mode()
def torch_get_similar_confidence_matrix2(seg_neighbors,
                                                   seg_ids,
                                                   seg_seen,
                                                   points_label,
                                                   ...
                                                   batch_size=200):
    # no use?
    device = seg_seen.device
    view_num, seg_num = seg_seen.shape[1], seg_seen.shape[0]

    gpu_seg_ids = seg_ids.to(device)
    gpu_points_label = points_label.to(device)
    gpu_seg_neighbors = seg_neighbors.to(device)
    gpu_seg_seen = seg_seen.to(device)

    similar_sum = torch.zeros((seg_num, seg_num), device=device)
    confidence_sum = torch.zeros((seg_num, seg_num), device=device)

    for m in tqdm(range(view_num)):
        plabels = gpu_points_label[:, m]
        label_range = int(plabels.max().item()) + 1
        if label_range < 2:
            continue

        seglabels = torch.zeros((seg_num, label_range), device=device)
        
        unique_labels_segids, unique_counts = torch.unique(
            torch.stack([gpu_seg_ids, plabels]), dim=1, return_counts=True)
        seglabels[unique_labels_segids[0], unique_labels_segids[1]] = unique_counts.float()

        nonzero_seglabels = seglabels[:, 1:]
        
        if similar_metric == "2-norm":
            norms = nonzero_seglabels.norm(dim=-1, keepdim=True).clamp_min(1e-8)
            nonzero_seglabels /= norms
        else:
            raise NotImplementedError
        
        view_confidence = gpu_seg_seen[:, m].unsqueeze(1)
        one_view_confidence = view_confidence @ view_confidence.T
        one_view_similar = torch.zeros_like(similar_sum)

        for start_id in range(0, seg_num, batch_size):
            end_id = min(start_id + batch_size, seg_num)
            neighbor_mask = gpu_seg_neighbors[start_id:end_id].any(dim=0)
            if not neighbor_mask.any():
                continue

            batch_labels = nonzero_seglabels[start_id:end_id]
            neighbors_labels = nonzero_seglabels[neighbor_mask]

            batch_similar = torch_calcu_all_similar_optimized(
                batch_labels, neighbors_labels, thres_trunc=thres_trunc)

            one_view_similar[start_id:end_id][:, neighbor_mask] = batch_similar
...
def torch_get_similar_confidence_matrix(seg_neighbors,
    ...
    gpu_seg_ids = seg_ids.to(device)  # (n,)
    gpu_points_label = points_label.to(device)  # (n,m)####
    gpu_seg_neighbors = seg_neighbors.to(device)  # (s,s)
    gpu_seg_seen = seg_seen.to(device)  # (s,m)####
    ...
    for m in tqdm(range(view_num)):
        ...
        for start_id in range(0, seg_num, batch_size):
            ...
            one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = \
                torch_calcu_all_similar_optimized(nonzero_seglabels[start_id:start_id+batch_size],
                                        neighbors_labels,
                                        thres_trunc=thres_trunc)
        # (s,1) @ (1,s) = (s,s)
    ...
    return similar_sum, confidence_sum#[similar_sum.cpu().numpy(), confidence_sum.cpu().numpy()]
...
def save_label_ply(point_xyz: np.ndarray, label: torch.Tensor, save_path: str):

    val2idx = label.unique().cpu()
    # rand color for each seg
    palette = []
    for i in range(len(val2idx)):
    ...
    label = label.cpu()
    vertex_data['x'] = point_xyz[:, 0]
    vertex_data['y'] = point_xyz[:, 1]
    vertex_data['z'] = point_xyz[:, 2]
    for i, val in enumerate(val2idx):
        idx = label == val
        vertex_data['red'][idx] = palette[i][0]
        vertex_data['green'][idx] = palette[i][1]
        vertex_data['blue'][idx] = palette[i][2]


    new_ply = plyfile.PlyData([
        plyfile.PlyElement.describe(vertex_data, 'vertex')
    ])
    new_ply.write(save_path)
...
if __name__ == "__main__":
    ...
    seq = np.array(seq)
    vn = 0
    print(num_to_natural(seq, void_number=vn))
    print(num_to_natural_torch(torch.tensor(seq), void_number=vn))
