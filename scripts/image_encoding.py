...
def create(image_list, data_list, save_folder):
    ...
    for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False):
        if os.path.exists(os.path.join(save_folder, data_list[i].split('.')[0] + '_f.npy')) and \
           os.path.exists(os.path.join(save_folder, data_list[i].split('.')[0] + '_s.npy')):
            print(f"Skipping {data_list[i]} since it already exists.")
            continue
        timer += 1
        ...
        try:
            ...
            print(f"Error in image {data_list[i]}")
            continue
...
if __name__ == '__main__':
    ...
    save_dir = args.save_dir if args.save_dir is not None else os.path.join(source_path, 'language_features')
    sam_ckpt_path = args.sam_ckpt_path
    img_folder = os.path.join(source_path, 'images')
    # img_folder = os.path.join(source_path, 'dslr/resized_undistorted_images')
    ...
    while True:
        try:   
            ...
            if os.path.exists(save_dir):
                processed_frames = [f.split('_f')[0] for f in os.listdir(save_dir) if f.endswith('_f.npy')]
                if len(processed_frames) >= len(data_listi):
                    break
            data_list = [frame for frame in data_listi if not frame.split('.')[0] in processed_frames]
            for data_path in data_list:
                ...
                # print(image.shape, resolution)
                img_list.append(image)
            images = [img_list[i].permute(2, 0, 1)[None, ...] for i in range(len(img_list))]
            imgs = torch.cat(images)
            print(f"resolution: {resolution}, first file{data_list[0]}, last file{data_list[-1]} with total length of {len(data_list)}")

            os.makedirs(save_dir, exist_ok=True)
            create(imgs, data_list, save_dir)
