...
from spt.utils.scatter import scatter_nearest_neighbor
...
def knn_1(
    ...
    # Data initialization
    device = xyz.device
    xyz_query = (xyz + batch_offset).view(1, -1, len_feat)
    xyz_search = (xyz + batch_offset).view(1, -1, len_feat)
    if not xyz.is_cuda:
    ...
    if not verbose and not spt.is_debug_enabled():
...
def knn_1_graph(
    ...
    if trim:
        from spt.utils import to_trimmed
